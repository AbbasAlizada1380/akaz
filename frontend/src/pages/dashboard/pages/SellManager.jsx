...
const SellManager = () => {
  ...
  const [items, setItems] = useState([
    { id: Date.now(), productId: "", productName: "", departmentId: "", amount: "", unitPrice: "", total: 0 },
  ]);
  ...
  const handleProductSelect = (itemId, productId) => {
    ...
    if (selectedProduct) {
      ...
      updateItem(itemId, "departmentId", selectedProduct.departmentId || "");
      updateItem(itemId, "unitPrice", defaultPrice);
    } else {
      updateItem(itemId, "productId", productId);
      updateItem(itemId, "productName", "");
      updateItem(itemId, "departmentId", "");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      items: items.map((item) => ({
        ...
        departmentId: item.departmentId ? parseInt(item.departmentId) : undefined, // include department
        total: item.total,
      })),
