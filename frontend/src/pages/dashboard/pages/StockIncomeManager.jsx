...
export default function StockIncomePage() {
    ...
    // Global department for all new products AND filter for existing products
    ...
    // Filter existing products based on selected global department
    const filteredExists = useMemo(() => {
        if (!globalDepartmentId) {
            // If no department selected, show all products (or return [] if you prefer)
            return exists;
        }
        return exists.filter(ex => String(ex.departmentId) === String(globalDepartmentId));
    }, [exists, globalDepartmentId]);
    ...
    // Sync global department to all rows that are "new product"
    useEffect(() => {
        setRows((prevRows) =>
            prevRows.map((row) => {
                if (!row.existId) {
                    return { ...row, department: globalDepartmentId };
                }
                return row;
            })
        );
    ...
    // Optional: clear existId from rows if the selected product no longer matches the filtered list
    useEffect(() => {
        setRows(prevRows =>
            prevRows.map(row => {
                if (row.existId && globalDepartmentId) {
                    const productExists = filteredExists.some(ex => String(ex.id) === String(row.existId));
                    if (!productExists) {
                        // Product no longer belongs to selected department – reset row
                        return {
                            ...row,
                            existId: "",
                            newExist: "",
                            department: globalDepartmentId,
                        };
                    }
                }
                return row;
    ...
    }, [globalDepartmentId, filteredExists]);
    ...
    const handleRowChange = (index, field, value) => {
        ...
        if (field === "existId") {
            newRows[index].existId = value;
            if (value) {
                // Clear new product name and set department from the chosen product
                newRows[index].newExist = "";
                const selectedProduct = filteredExists.find((ex) => String(ex.id) === String(value));
                newRows[index].department = selectedProduct?.departmentId || "";
            } else {
                // Switching back to new product – apply global department
                newRows[index].department = globalDepartmentId;
            }
        } else {
    ...
    const handleSubmit = async (e) => {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            if (!row.existId && row.newExist.trim() && !row.department) {
                alert(`Row ${i + 1}: Please select a department (global) for the new product`);
                return;
            }
        ...
        const payload = {
            ...
            paidAmount: paid,
            departmentId: globalDepartmentId || null,
        };
            ...
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
                    <form onSubmit={handleSubmit} className="divide-y divide-gray-200">
                            ...
                            <div className="space-y-6">
                                <div className="flex flex-wrap gap-4">
                                    <label className="inline-flex items-center">
                                        ...
                                        />
                                        <span className="ml-2 text-sm text-gray-700">Existing Seller</span>
                                    </label>
                                    <label className="inline-flex items-center">
                                        <input
                                            type="radio"
                                            ...
                                            className="h-4 w-4 text-cyan-600 focus:ring-cyan-500 border-gray-300"
                                        />
                                        <span className="ml-2 text-sm text-gray-700">New Seller</span>
                                    </label>
                                </div>
                                ...
                                {sellerMode === "new" && (
                                    <div className="max-w-md">
                                        <label className="block text-sm font-medium text-gray-700 mb-1">New Seller Name *</label>
                                        <input
                                            type="text"
                        ...
                        {/* Items Section with global department control */}
                        <div className="px-6 py-6 sm:px-8">
                            ...
                            <div className="mb-6 p-4 bg-gray-50 rounded-lg border border-gray-200">
                                <label className="block text-sm font-medium text-gray-700 mb-2">
                                    Global Department (applies to all new products & filters existing products) *
                                </label>
                                <select
                                    value={globalDepartmentId}
                                    onChange={(e) => setGlobalDepartmentId(e.target.value)}
                                    required
                                    className="block w-full max-w-md rounded-md border-gray-300 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 sm:text-sm"
                                >
                                ...
                                <p className="text-xs text-gray-500 mt-1">
                                    This department will be used for new products and filters the existing product dropdown.
                                </p>
                            </div>
                                ...
                                <table className="min-w-full divide-y divide-gray-200 text-sm">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">New Product</th>
                                            ...
                                            <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Remove</th>
                                        </tr>
                                    </thead>
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {rows.map((row, idx) => {
                                            ...
                                            if (row.existId) {
                                                const product = filteredExists.find((ex) => String(ex.id) === String(row.existId));
                                                departmentDisplay = product?.department?.name || getDepartmentName(product?.departmentId) || "—";
                                            } else {
                                                departmentDisplay = getDepartmentName(row.department) || "—";
                                            }
                                            return (
                                                <tr key={idx}>
                                                    <td className="px-4 py-2">
                                                        <select
                                                            value={row.existId}
                                                            onChange={(e) => handleRowChange(idx, "existId", e.target.value)}
                                                            className="block w-40 rounded-md border-gray-300 shadow-sm focus:border-cyan-500 focus:ring-cyan-500 sm:text-sm"
                                                        >
                                                            <option value="">Select product</option>
                                                            {filteredExists.map((ex) => (
                                                                <option key={ex.id} value={ex.id}>
                                                                    {ex.name} ({ex.department?.name || getDepartmentName(ex.departmentId)})
                                                                </option>
                                                            ))}
                                                        </select>
                                                    </td>
                                                    ...
                                                    <td className="px-4 py-2">
                                                        <div className="text-sm text-gray-700 px-2 py-1 bg-gray-50 rounded">
                                                            {departmentDisplay || "—"}
                                                        </div>
                                                    </td>
                                                    <td className="px-4 py-2">
                        ...
                        <div className="px-6 py-6 sm:px-8 bg-gray-50">
                            <h2 className="text-lg font-medium text-gray-900 mb-4">Payment to Seller</h2>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        required
                                    />
                                    <p className="text-xs text-gray-500 mt-1">Amount actually paid (can be partial)</p>
                                </div>
                            </div>
                        </div>
                ...
                {activeTab === "stockIncome" ? (
                    <div className="bg-white rounded-2xl shadow-xl overflow-hidden mt-12">
                        <div className="px-6 py-5 border-b border-gray-200 flex justify-between items-center">
                            <h2 className="text-lg font-medium text-gray-900">Recent Stock Incomes</h2>
                            ...
                            <table className="min-w-full divide-y divide-gray-200">
                                <thead className="bg-gray-50">
                                    <tr>
                                        ...
                                        <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Net Unit Price</th>
                                ...
                                <tbody>
                                    {incomes.map((inc) => (
                                        <tr key={inc.id} className="hover:bg-gray-50 transition">
                                            <td className="px-6 py-3 text-sm text-gray-800">{inc.seller?.fullname || inc.seller?.name}</td>
                                            <td className="px-6 py-3 text-sm text-gray-800">{inc.stock?.name || `Product ID: ${inc.existId}`}</td>
                                            <td className="px-6 py-3 text-sm text-gray-600">{inc.type}</td>
                                            <td className="px-6 py-3 text-sm text-gray-600">{inc.amount}</td>
                                            <td className="px-6 py-3 text-sm text-gray-600">{inc.net_unite_price}</td>
                                    ...
                                    {incomes.length === 0 && (
                                        <tr>
                                            <td colSpan="7" className="px-6 py-12 text-center text-gray-400">No income records found</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
